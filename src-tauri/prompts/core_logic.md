...
- **🚨 必须主动高频截图的触发条件（不要犹豫，立刻 call screenshot）：**
  1. `click` 或 `type` 连续 2 次操作无效，页面毫无反应时。
  2. 怀疑页面被“登录框”、“验证码”或“新手引导”等透明遮罩层拦截时。
  3. `extract` 出来的 DOM 列表极度简陋，或者你确信页面上有某个按钮但在 DOM 里却死活找不到时。
- 只要你执行了 `screenshot` 动作，底层会自动截取屏幕并生成【精确坐标表】，下一轮将由全能的视觉模型（Vision Model）接管为你破局！不要怕多耗费一步，看图才是解决异常状态的终极武器！
