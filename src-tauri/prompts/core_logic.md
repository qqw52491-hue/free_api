...
- **特别注意**：调用 `finish` 时，必须在 `command` 中以 JSON 格式提交你的最终成果（如：生成的 Excel 文件路径、采集到的核心数据摘要、任务总结等）。这将作为任务的最终交付物呈现给用户。示例见场景 C。
